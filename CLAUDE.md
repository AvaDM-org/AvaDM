...
- `test/AvaDM.UI.Tests` — xUnit tests for UI view-model logic (currently `DownloadColumnsViewModelTests`).
...
- Polly retries transient connection errors, I/O errors, inactivity timeouts, HTTP 408/429/5xx responses, with exponential backoff and jitter. Defaults to five retries; configurable via `DownloadSettings.DefaultMaxRetryAttempts` and `DefaultRetryBaseDelay`. The resilience pipeline (`ChunkResiliencePipelineFactory`) is retry-only - no fixed per-execution timeout - and is built fresh per download from current settings, so changes take effect on the next download started.
- Timeouts are stall-based, not attempt-duration-based: `Downloader`'s `SendWithStallWatchdogAsync`/`ReadWithStallWatchdogAsync` wrap the connect/headers phase and every body read in a `CancellationTokenSource` that's reset (`CancelAfter`) on every byte received, converting only genuine inactivity - never outer cancellation - into `TimeoutRejectedException` for Polly to retry. `DownloadSettings.DefaultInactivityTimeout` (default 30s) is how long an attempt can go without receiving anything, not a cap on its total duration - a slow-but-steady transfer runs as long as it takes. This matters most for a non-resumable whole-file download (no `Content-Length`, or no Range support): a retry there always restarts from byte 0, so a flat attempt-duration timeout shorter than the real transfer time would make it fail identically forever instead of ever completing. `DownloadManager`'s auto-retry (`DefaultAutoRetryAttempts`) doesn't fully help a repeatedly-failing whole-file download either - unlike a ranged chunk's footer-based resume, each automatic retry there is also a full restart from byte 0.
...
- **Downloads page** (`DownloadListViewModel`/`DownloadRowViewModel`) — a file-explorer-style table (redesigned in issue #19; sketch in `docs/redesign/`). Two toolbar bars: bar 1 has a debounced text search, an advanced-add `+` button, a quick-add link box (clipboard-paste icon inside, start button after — reuses `AddDownloadViewModel`'s submit, so a bad URL or a conflict opens the advanced dialog), a selection trash icon, and the settings gear (the advanced-add dialog also has "Import list from clipboard/file" buttons that open `BulkImportViewModel`: one link per line via `BulkImportParser`, per-link checkboxes, one shared editable save folder, each ticked link added through `DownloadManager.AddDownloadAsync` so the queue decides what starts); bar 2 is the column header. Columns (`DownloadColumnsViewModel`/`DownloadColumnViewModel`, `DownloadColumnId`): Name pinned, plus optional Type/Size/Created/Speed/Progress %/Progress size/Status — show/hide + reorder via header right-click or drag, resize via an edge grip, sort by clicking a header. Column order/visibility/sort persist as JSON under `UiPreferencesRepository.DownloadListLayoutKey`; widths don't. Rows are full-bleed, multi-selectable (click/Ctrl/Shift/checkbox/select-all → `SelectedDownloads`); the trash icon or Delete removes the whole selection via `BulkRemoveConfirmationViewModel` (lists the names, one "delete files" toggle). Each row: a fixed-width strip of the 5 action icons (pause/resume/cancel/open-folder/remove) after the name/path, the aggregate progress bar (a `SegmentedProgressBar` — one uneven-fill segment per connection; **no row expansion any more**) for active rows only, and one `DownloadCellViewModel` per visible column. Right-click a row for open file / open folder / copy link / remove-selection. A row with no live handle for this process shows as a derived **Interrupted** status; downloads are not yet auto-resumed on startup, see Roadmap. Cancel/Remove/bulk-remove go through confirmation overlays; non-terminal `DownloadHandle.LogMessage` events surface as toasts. A 1-second reconciliation poll keeps the list in sync with downloads started/finished elsewhere.
- **Settings page** (`SettingsViewModel`) — staged edits over the shared `DownloadSettings` (download dir, connection count — "Connections per download", still `DefaultChunkCount` in code — retries, speed limit, repository path), plus UI-only preferences: appearance (dark/light), window-closing behavior (minimize to tray vs. close), completed-item double-click action, start-with-system (autostart), and a log-folder shortcut.
...
2. Develop and test on that branch. Commit with Conventional Commits (`fix(core): ...`, `feat(ui): ...`, `test(core): ...`), **one small commit per logical step** — never a single big catch-all commit. Build and run the tests before each commit. Add the change to `CHANGELOG.md` under the current next-release section (see below).
3. Open a PR against `master` with `gh pr create`, referencing the issue (e.g. "Fixes #10").
4. **Wait for the user to test.** Do not tag, or merge on your own initiative. The user tests the branch and tells you when it's good and whether to merge.
5. Once the user confirms, `git push` any final commits, merge the PR into `master` (the user merges via the GitHub UI unless they explicitly ask this session to merge), then fast-forward local `master`: `git fetch && git checkout master && git merge --ff-only origin/master`.

### Changelog and releasing

- A release usually bundles **several merged PRs**. The changelog section for the next release is opened as `## [X.Y.Z] - Unreleased` as soon as the first change since the last tag lands, and every PR after that adds its entry there. Ask the user for the target version when opening the section if it isn't obvious.
- Merging a PR **never** triggers a release — `release.yml` only fires on a `vX.Y.Z` tag push.
- **Only when the user explicitly asks to cut the release:** open a tiny `chore/release-X.Y.Z` branch that swaps that section's `Unreleased` for the date, PR it, merge it, fast-forward `master`, then `git tag vX.Y.Z && git push origin vX.Y.Z` on that merged commit. The tag must point at a commit already on `master` that contains its own dated changelog section, or the release build fails.
